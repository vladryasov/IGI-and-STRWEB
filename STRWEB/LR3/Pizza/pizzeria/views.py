...
from .models import Pizza, Review, PromoCode, Vacancy, UserProfile, PickupPoint, PizzaCategory, Order, OrderItem, Coupon, PizzaSize, Contact, News, Partner, CompanyInfo, CompanyHistory
...
def about(request):
    ...
    company_history = CompanyHistory.objects.all().order_by('year') if company_info else []
...
def seed_contacts_if_needed():
    """Создаёт недостающие контакты, чтобы было минимум 10 записей."""
    ...
    samples = [
        {
            'address': 'Иван Петров',
            'phone': '+375291112233',
            'email': 'ivan.petrov@pizzeria.by',
            'working_hours': 'Пиццамейкер, тесто и печь',
            'map_link': 'https://pizzeria.by/staff/ivan',
            'is_main': True,
        },
        {
            'address': 'Анна Смирнова',
            'phone': '+375331122334',
            'email': 'anna.smirnova@pizzeria.by',
            'working_hours': 'Кондитер, десерты и сезонные новинки',
            'map_link': 'https://pizzeria.by/staff/anna',
            'is_main': False,
        },
        {
            'address': 'Дмитрий Соколов',
            'phone': '+375251234567',
            'email': 'd.sokolov@pizzeria.by',
            'working_hours': 'Доставщик, оперативная логистика',
            'map_link': 'https://pizzeria.by/staff/dmitry',
            'is_main': False,
        },
        {
            'address': 'Мария Иванова',
            'phone': '+375441112233',
            'email': 'm.ivanova@pizzeria.by',
            'working_hours': 'Оператор, помощь клиентам',
            'map_link': 'https://pizzeria.by/staff/maria',
            'is_main': False,
        },
        {
            'address': 'Сергей Ковалёв',
            'phone': '+375291234567',
            'email': 's.kovalev@pizzeria.by',
            'working_hours': 'Менеджер смены, организация кухни и зала',
            'map_link': 'https://pizzeria.by/staff/sergey',
            'is_main': False,
        },
        {
            'address': 'Екатерина Орлова',
            'phone': '+375331234568',
            'email': 'e.orlova@pizzeria.by',
            'working_hours': 'Бариста, напитки и кофе',
            'map_link': 'https://pizzeria.by/staff/ekaterina',
            'is_main': False,
        },
        {
            'address': 'Алексей Никифоров',
            'phone': '+375251122334',
            'email': 'a.nikiforov@pizzeria.by',
            'working_hours': 'Закупки и склад',
            'map_link': 'https://pizzeria.by/staff/alexey',
            'is_main': False,
        },
        {
            'address': 'Юлия Кравцова',
            'phone': '+375441234569',
            'email': 'y.kravtsova@pizzeria.by',
            'working_hours': 'HR и обучение персонала',
            'map_link': 'https://pizzeria.by/staff/yulia',
            'is_main': False,
        },
        {
            'address': 'Николай Васильев',
            'phone': '+375171234560',
            'email': 'n.vasilev@pizzeria.by',
            'working_hours': 'Техническое обслуживание печей',
            'map_link': 'https://pizzeria.by/staff/nikolay',
            'is_main': False,
        },
        {
            'address': 'Ольга Матвеева',
            'phone': '+375291110000',
            'email': 'o.matveeva@pizzeria.by',
            'working_hours': 'SMM и маркетинг',
            'map_link': 'https://pizzeria.by/staff/olga',
            'is_main': False,
        },
    ]
    ...
    for sample in samples:
        if Contact.objects.filter(phone=sample['phone']).exists():
            continue
        to_create.append(Contact(**sample))
...
def add_to_cart(request, pizza_id):
    ...
    multiplier = Decimal(str(size_obj.price_multiplier))
    price_with_size = pizza.base_price * multiplier
    ...
    order_item, created = OrderItem.objects.get_or_create(
        ...
        defaults={
            'quantity': quantity,
            'price': price_with_size
        }
    )
    
    if not created:
        order_item.quantity += quantity
        order_item.price = price_with_size
        order_item.save()
...
def partners(request):
    ...
    return render(request, 'pizzeria/partners.html', {
...
def employees_table(request):
    ...
    return render(request, 'pizzeria/employees_table.html')
...
def form_generator(request):
    """Страница с генератором формы"""
    return render(request, 'pizzeria/form_generator.html')

def scroll_animation(request):
    """Страница с анимацией при скроллинге"""
    return render(request, 'pizzeria/scroll_animation.html')

def age_validator(request):
    """Страница валидации возраста"""
    return render(request, 'pizzeria/age_validator.html')

def library_readers(request):
    """Страница с читателями библиотеки"""
    return render(request, 'pizzeria/library_readers.html')

@user_passes_test(lambda u: is_admin(u), login_url='/no-access/')
def geolocation(request):
    """Страница геолокации"""
    return render(request, 'pizzeria/geolocation.html')

@user_passes_test(lambda u: is_admin(u), login_url='/no-access/')
def chart_arccos(request):
    """Страница с графиками arccos x"""
    return render(request, 'pizzeria/chart_arccos.html')

def no_access(request):
    """Страница для пользователей без доступа"""
    message = "У вас нет доступа к этой странице. Требуются права администратора."
    if not request.user.is_authenticated:
        message = "Пожалуйста, войдите в систему для доступа к этой странице."
    return render(request, 'pizzeria/no_access.html', {'message': message})